...
def init_models(app, db):
    ...
    with app.app_context():
        ...
        # First initialize Subscription model since User model depends on it
        ...
        # Re-check User model's relationships after all models are initialized
        if hasattr(User, "verify_relationships"):
            try:
                User.verify_relationships()
                app.logger.info("User relationships verified")
            except Exception as e:
                app.logger.error(f"Error verifying User relationships: {str(e)}")
