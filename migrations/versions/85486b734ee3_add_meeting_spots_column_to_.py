...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("new_id", postgresql.UUID(), nullable=False),
    )
    ...
    op.add_column("meeting_requests", sa.Column("uuid_user_a_id", postgresql.UUID(), nullable=True))
    ...
    op.add_column("meeting_requests", sa.Column("meeting_spots", postgresql.JSONB(), nullable=True))
...
def downgrade():
    ...
    op.create_table(
        "id_mapping",
        sa.Column("uuid_id", postgresql.UUID(), nullable=False),
    ...
    # Update the temporary integer column with new values
    op.execute(
        """
        UPDATE users u
        SET int_id = nextval('users_id_seq')
        FROM id_mapping m
    ...
    # Make the id column NOT NULL and add a primary key constraint
    op.execute("ALTER TABLE users ALTER COLUMN id SET NOT NULL")
